...
class ActivityQuizManageTimingEnforcedEditor extends AsyncContainerMixin(LocalizeActivityQuizEditorMixin(SkeletonMixin(ActivityEditorMixin(MobxLitElement)))) {
	static get styles() {
		return [
			super.styles,
			inputLabelStyles,
	...
	render() {
		...
		return html`
			${this._renderTimeEnforcedEditor(entity)}
		`;
	}

	_renderExtendedDeadlineEditor(entity) {
		const {
	...
	_renderTimeEnforcedEditor(entity) {
		const {
			submissionLateType,
			enforcedTimeLimit,
			enforcedGraceLimit
		...
		return html`
			<div class="d2l-time-menu-container">
				...
				${this._renderExtendedDeadlineEditor(entity)}
			</div>
		`;
	}
...
customElements.define(
	...
	ActivityQuizManageTimingEnforcedEditor
);
